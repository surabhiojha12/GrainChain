function seeDistributors(){
    let url = "http://localhost:3000/api/queries/selectDistributorsParticipant"
    axios.get(url)
    .then((response) => {
    ...
    displayDistributors(response.data);
  });
}
...
function displayDistributors(data){
    ...
    for(producer in data){
        ...
        "<h2><strong>Distributor Id </strong>" + field.traderId +"</h2>" +
...
if(id== -1){
    document.getElementById("display-distributors").innerHTML = htmlData;}
else{
