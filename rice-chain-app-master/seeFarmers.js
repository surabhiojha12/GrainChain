function seeFarmers(){
    let url = "http://localhost:3000/api/queries/selectFarmersParticipant"
    axios.get(url)
    .then((response) => {
...
function displayFarmers(data){
    ...
    for(producer in data){
        ...
        "<h2><strong>Farmer Id </strong>" + field.traderId +"</h2>" +
...
document.getElementById("display-farmers").innerHTML = htmlData;
}